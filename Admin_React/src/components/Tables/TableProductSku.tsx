...
import { deleteProductSku } from '../../api';
...
const TableProductSku = ({ productDto, fetchDelete }: Props) => {
        ...
        <div className="rounded-sm px-6 py-5 pb-4 shadow-default dark:border-strokedark dark:bg-boxdark">
            <div className="max-w-full overflow-x-auto text-center">
                <table className="w-full table-auto">
                    ...
                    <tbody>
                        {!productDto.productSkusDto ? <></> : productDto.productSkusDto.map((productSkuDto, index) => (
                            <tr key={index}>
                                ...
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {productSkuDto.unitsInStock}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white">
                                        {productSkuDto.unitsSold}
                                    </p>
                                </td>
                                ...
                                <td className="border-b border-[#eee] py-5 px-2 dark:border-strokedark">
                                    <div className="flex justify-center space-x-3.5">
                                        <Link
                                            to={
                                                `/productSku/form?productId=${productSkuDto.productId}` //Update
                                            }
                                            state={productSkuDto}
                                            className='block'
                                        >
                                        ...
                                        <button
                                            onClick={() => handleClickDelete(productSkuDto.id)}
